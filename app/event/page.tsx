...
export default function Home() {
    ...
    const handleSubmit = async (event:any) => {
        ...
        const id = (name.toLowerCase().replace(/\s/g, '-'))+'-'+dayjs(date).format('YYYY-MM-DD');
    ...
    <IconButton href="/" color="primary">
      ...
      <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1">
                ...
                <TextField required id="eventName" placeholder="ACM Event" fullWidth />
            </div>

            <LocalizationProvider dateAdapter={AdapterDayjs}>
                <div className="flex flex-col gap-1">
                <label htmlFor="eventDate" className="font-medium">Event Date*</label>
            ...
            <div className="flex flex-col gap-1">
                <label htmlFor="eventCover" className="font-medium">Event Cover Image*</label>
                <MuiFileInput id="eventCover" placeholder="Upload .jpg, .jpeg or .png" required value={file} onChange={(newValue)=>setFile(newValue)} />
            </div>

            <div className="flex flex-col gap-1">
                <label htmlFor="eventLink" className="font-medium">Event Registration Link{dateAfter?'*':''}</label>
                <TextField id="eventLink" required={dateAfter} placeholder="Google Form Link" fullWidth />
            </div>

            <div className="flex flex-col gap-1 md:col-span-2">
                <label htmlFor="eventDescription" className="font-medium">Event Description*</label>
                <TextField required id="eventDescription" multiline minRows={4} placeholder="1 or 2 paragraphs about the event." fullWidth />
            </div>
